    ...
    const fetchOrders = async () => {
        try {
            ...
            const transformedOrders = ordersData.map(order => ({
                ...
                customerEmail: order.userId, // In real app, you'd fetch user email
                customerPhone: 'N/A', // Not available in current schema
                products: order.items?.map(item => ({
                    name: item.name || 'Gold Jewelry Item',
                    quantity: item.quantity || 1,
                    price: item.price || (order.total / (item.quantity || 1)),
                    description: item.description || 'Premium quality gold jewelry'
                })) || [],
                totalAmount: order.total,
                status: order.status,
                paymentMethod: order.paymentMethod === 'phonepe' ? 'PhonePe' : order.paymentMethod,
                paymentStatus: order.status === 'Cancelled' ? 'Cancelled' : (order.transactionId ? 'Paid' : 'Pending'),
                shippingAddress: order.deliveryAddress ? {
                    street: order.deliveryAddress,
                    city: 'Chennai', // Default values since schema doesn't separate address
                    state: 'Tamil Nadu',
                    pincode: '600001'
                } : null,
            ...
            calculateOrderStats(transformedOrders);
